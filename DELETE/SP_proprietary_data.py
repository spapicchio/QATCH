...
from utils import init_metric_evaluator, get_predictions_results_from_dbs
...
def main(args):
    ...
    tests_df = pd.read_json('SPIDER_OUR_TESTS/SP/spider_custom_tests_resdsql.json')
    ...
    """
    # model = init_model('chatgpt', test_type_chatgpt='SP')
    if args.verbose:
    ...
    #     lambda row: model.predict(table=tables[row.tbl_name],
    #                               queries=row.question,
    #                               tbl_name=row.tbl_name),
    #     axis=1
    # )
    # # TODO: delete
    # tests_df.reset_index(drop=True).to_json(
    #     f'experiments_SP/chatgpt/tests_{args.db_category}_pred.json'
    # )
    # TODO --------
    # 3. QATCH-Evaluate
    """
    ...
    """
    args.output_db_path = './data/spider/database'
    tests_df = get_predictions_results_from_dbs(base_path_db=args.output_db_path,
                                                df=tests_df,
                                                predictions='predictions_resdsql')
    name = 'resdsql'
    tests_df.rename({'query_result_predictions': f'query_result_predictions_{name}'},
                    axis=1, inplace=True)
    ...
    tests_df = metric_evaluator.evaluate_with_df(tests_df,
                                                 target='query_result',
                                                 predictions='query_result_predictions_resdsql',
                                                 task='SP')

    tests_df.rename({'cell_precision': f'cell_precision_{name}',
                     'cell_recall': f'cell_recall_{name}',
                     'tuple_constraint': f'tuple_constraint_{name}',
                     'tuple_order': f'tuple_order_{name}',
                     'tuple_cardinality': f'tuple_cardinality_{name}'},
                    inplace=True, axis=1)
    # 4. save tests results and summary
    # path = f'{args.output_db_path}/{args.db_category}/tests_with_results.json'
    path = f'SPIDER_OUR_TESTS/SP/spider_custom_tests_resdsql_eval.json'
...
if __name__ == '__main__':
    ...
    parser.add_argument('--sql_generators', default=['groupby', 'having', 'simpleAgg'], nargs='*',
