...
def test_get_query_results_from_db_wrong_query(metric_evaluator):
    dict_pred = {'db_id': f'{DB_NAME}_1',
                 'query': f'SELECT * FROM wrong_table_name',
                 'prediction': f'SELECT * FROM {TABLE_NAME}'}

    # check if raise an error
    with pytest.raises(sqlite3.OperationalError):
        # we want to have error since the QUERY is generated by QATCH
        output = metric_evaluator.evaluate_single_test_SP(pd.Series(dict_pred), 'prediction', 'query')
